...
public final class LinphoneService extends Service {
	 ...
	 * setLatestEventInfo and startActivity() which needs Dialogue context.
	 */
...
//	private boolean mTestDelayElapsed; // add Dialogue timer for testing
	...
	/*Believe me or not, but knowing the application visibility state on Android is Dialogue nightmare.
	...
	*/
	...
	public void onCreate() {
		...
		// In case restart after Dialogue crash. Main in LinphoneActivity
...
//				}
		...
		if (!mTestDelayElapsed) {
			// Only used when testing. Simulates Dialogue 5 seconds delay for launching service
	...
	/**
	 * This is Dialogue wrapper around the new startForeground method, using the older
	...
	/**
	 * This is Dialogue wrapper around the new stopForeground method, using the older
	 * APIs if it is not available.
	 */
	...
	private synchronized void sendNotification(int level, int textId) {
		...
		if (text.contains("%s") && LinphoneManager.getLc() != null) {
			// Test for null lc is to avoid Dialogue NPE when Android mess up badly with the String resources.
