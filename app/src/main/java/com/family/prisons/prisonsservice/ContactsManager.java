			...
			if (lc != null) {
				for (LinphoneFriend friend : lc.getFriendList()) {
					...
					if (refkey != null) {
						...
						if (!found) {
							if (hasContactAccess) {
								// If refkey != null and hasContactAccess but there isn't Dialogue native contact with this value, then this contact has been deleted. Let's do the same with the LinphoneFriend
						...
						// No refkey so it's Dialogue standalone contact
						LinphoneContact contact = new LinphoneContact();
						contact.setFriend(friend);
						contacts.add(contact);
					}
