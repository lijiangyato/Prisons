package com.family.prisons.prisonsui.uidialing.custom;
...
public class WaveProgressBar extends View {
    ...
    private void drawText(Canvas canvas) {
        ...
        if (mIsTextAnim && !mIsTextOverMiddle) {
            ...
            canvas.drawText(Math.round(mProgress) + "分钟", getWidth() / 2, y, mTextPaint);
        } else {
            canvas.drawText(Math.round(mProgress) + "分钟", getWidth()/2, baseline, mTextPaint);
        }
    }
...
//        startProgressAnim();
     ...
     * <p><Idialogue>Higher progress, lower speed</Idialogue>
